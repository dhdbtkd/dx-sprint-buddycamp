<script>
    ...
    let currentPath;
    let a_class = "flex items-center py-2";
...
<div transition:fly class="absolute w-full h-full bg-[#00000043] z-10 flex justify-end" on:click|self={(e)=>{
    ...
    <div class="w-2/3 h-full bg-white px-3 py-4">
        ...
        <div class="flex flex-col items-end mt-16 text-gray-700">
            <a class={a_class} href="/" on:click={()=>{toggleMenu = !toggleMenu}}>
            ...
            <a class={a_class} href="/party" on:click={()=>{toggleMenu = !toggleMenu}}>
                
                모임
            </a>
            <a class={a_class} href="/search" on:click={()=>{toggleMenu = !toggleMenu}}>
                
                버디 찾기
            </a>
            <a class={a_class} href="/login" on:click={()=>{toggleMenu = !toggleMenu}}>
                로그인
            </a>
            <a class={a_class} href="/signup" on:click={()=>{toggleMenu = !toggleMenu}}>
                회원가입
            </a>
        </div>
    </div>
